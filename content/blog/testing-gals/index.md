...
Minipro has a test feature for common logic chips, which you invoke with `-T`. For example, if I put a 74LS14 hex inverter into my programmer I can test it by specifying part `7414`.
...
- `0` and `1` specify **input** lines that will be driven low or high.
- `V` and `G` specify the VCC and ground pins.
- `H` and `L` specify expected **outputs** that will be checked.

Test cases are run in order from top to bottom (several times around to catch flaky tests).

This test and many others are defined in the `logicic.xml` file that minipro installs. On my machine it's located in `/opt/homebrew/share/minipro/`. Here's the specification for `7414` and a bunch of other inverters with the same pinout. Note that the `<vector>` elements exactly match the test output above.
...
And here is a custom test database with a specification for our GAL. We place this test in a new file of our choosing (we don't need to modify the shared file).
...
- We specify `name="gp1"` and `type="5"` on the `<ic>` element. The name is arbitrary but it needs to be unique among the `<ic>` elements in the file. The type is required and is always `5`.
- All the `<vector>` elements in the shared database have an `id` element, but it's ignored so I leave it out.
- The test vectors use some tokens not discussed above:
...
- It's an error if you don't have enough tokens, but if you have too many it silently becomes confused and you will end up with weird test behavior. So double-check the token count if you can't figure out what's happening.
